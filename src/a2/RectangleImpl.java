...
public class RectangleImpl implements Rectangle, Polygon{
	...
	public RectangleImpl(Point p1, Point p2) {
		rpoints = new Point[]{p1,p2};
		points = new Point[] {getUpperLeft(), getUpperRight(), getLowerRight(), getLowerLeft()};
	}
	...
	public Point getLowerLeft() {
		double tempx = rpoints[0].getX();
		double tempy = rpoints[0].getY();
		for(Point p: rpoints){
			if(p.getX() < tempx){
				tempx = p.getX();
			}
	...
	public boolean isSquare() {
		if(getUpperLeft().distanceTo(getUpperRight()) == getUpperLeft().distanceTo(getLowerLeft())){
			return true;
		}
		else
			return false;
